...
/* v18: توحيد تنظيف الهاتف — حذف كل ما ليس رقماً + معالجة بادئة 00 الدولية */
...
router.post('/login', async (req, res) => {
  try {
    ...
    const user = await User.findOne({ phone });
    /* v18: رسائل خطأ دقيقة بدل «بيانات خاطئة» العامة */
    if (!user)
      return res.status(404).json({ message: 'هذا الرقم غير مسجّل لدينا — تحقق من الرقم أو أنشئ حساباً جديداً' });
    if (!(await bcrypt.compare(String(req.body?.password || ''), user.password)))
      return res.status(401).json({ message: 'كلمة المرور غير صحيحة — تأكد منها أو استخدم «استعادة»' });
...
router.post('/admin-login', async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(404).json({ message: 'لا يوجد حساب مدير بهذا الرقم' });
    if (!(await bcrypt.compare(String(req.body?.password || ''), user.password)))
